//! The interactive ratatui terminal help browser (`rusty2600 help --interactive`).
//!
//! Native-only, behind the default-on `help-tui` feature. A minimal first cut: a left-pane topic
//! list + a right-pane body, navigable with the arrow keys / `j`/`k`, `q` to quit. The topic text
//! comes from [`crate::cli::topic_text`] so the TUI and the static `help <topic>` output never
//! drift.
//!
        ...
        term.draw(|f| {
            ...
            let para = Paragraph::new(Text::raw(body))
                .block(
                    Block::default()
                        .borders(Borders::ALL)
                        .title(format!("Rusty2600 help — {}", TOPICS[selected])),
                )

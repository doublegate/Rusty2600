...
pub enum MenuAction {
    ...
    ///
    /// Native-only as of `[v2.9.0]`: the dispatch arm (`app.rs`) uses `rfd::FileDialog`'s
    /// native-only save dialog, which doesn't exist on wasm32 at all (see `Cargo.toml`'s `rfd`
    /// dependency gate) — enabling `debug-hooks` for the wasm32 debugger overlay therefore needs
    /// this one TAStudio action excluded specifically, while the four core CPU/TIA/RIOT/Memory
    /// panels (and TAStudio's own jump-to-frame) stay fully functional. See
    /// `debugger::tastudio_panel::render_tastudio_panel`'s wasm32 branch, which hides the "Save"
    /// button itself rather than leaving a dead click.
    ...
    /// Settings -> System -> "Generate share link" (`[v2.9.0]`): re-encode the live `Config` into
    /// a `?settings=` URL (`crate::share_link::share_url`) and stash it in
    /// [`ShellState::share_link`] so the Settings window can display it for the user to
    /// select-and-copy. Wasm32-only — native has no page URL to build one from.
    #[cfg(target_arch = "wasm32")]
    GenerateShareLink,
}
...
pub struct ShellState {
    ...
    /// The last share-link URL generated by Settings -> System -> "Generate share link"
    /// (`[v2.9.0]`, `MenuAction::GenerateShareLink`), displayed in a selectable text field so the
    /// user can copy it. Empty until the button is first clicked.
    #[cfg(target_arch = "wasm32")]
...
impl ShellState {
    ...
    /// sessions).
    // `[v2.9.0]`'s wasm32-only share-link section pushed this past the `too_many_lines`
    // threshold; it's still one cohesive "render the tabbed Settings window" function, so
    // splitting it apart for the lint alone would scatter tightly-related UI code.
    #[allow(clippy::too_many_lines)]
    fn render_settings(
        &mut self,
        ctx: &egui::Context,
            ...
            .show(ctx, |ui| {
                ...
                match self.settings_tab {
                    ...
                    _ => {
                        ...
                        // `[v2.9.0]` share-link: encode the CURRENT `cfg` (region + video +
                        // audio + key bindings) into a `?settings=` URL a user can paste to
                        // share their setup — see `crate::share_link`'s module doc. Wasm32-only
                        // (native has no page URL); the actual `window.location` read happens in
                        // `app.rs`'s dispatch (this shared module never calls a browser API
                        // directly), triggered by this button via `MenuAction::GenerateShareLink`.
                        #[cfg(target_arch = "wasm32")]
                        {
                            ui.separator();
                            ui.label("Share this configuration via URL:");
                            if ui.button("Generate share link").clicked() {
                                actions.push(MenuAction::GenerateShareLink);
                            }
                            if !self.share_link.is_empty() {
                                ui.horizontal(|ui| {
                                    ui.label("Link:");
                                    ui.add(
                                        egui::TextEdit::singleline(&mut self.share_link)
                                            .desired_width(ui.available_width()),
                                    );
                                });
                                ui.small(
                                    "Select the text above and copy it (Ctrl+C / Cmd+C) to share.",
                                );
                            }
                        }
                    }
    ...
    /// `debug-hooks` on, `actions` genuinely IS pushed into below; without it — a debugger-free
    /// build (`--no-default-features --features wasm-winit,help-tui,emu-thread`, see
    ...
    ///
    /// `[v2.9.0]`: `debug-hooks` is now confirmed wasm32-safe for `wasm-winit` too (`cargo
    /// clippy --target wasm32-unknown-unknown --no-default-features --features
    /// wasm-winit,debug-hooks`) — the core CPU/TIA/RIOT/Memory panels plus every other panel
    /// below work identically on both targets. The one exception is `TastudioSaveBranch`
    /// (`rfd`'s native-only save dialog), which is `not(target_arch = "wasm32")`-gated on its
    /// own (see `MenuAction::TastudioSaveBranch`'s doc comment) rather than excluding all of
                            ...
                            for action in tastudio_actions {
                                match action {
                                    crate::debugger::tastudio_panel::TastudioAction::JumpToFrame(
                                        idx,
                                    ) => actions.push(MenuAction::TastudioJumpToFrame(idx)),
                                    // Native-only (see `MenuAction::TastudioSaveBranch`'s doc
                                    // comment: its dispatch arm needs the native-only `rfd` save
                                    // dialog). `render_tastudio_panel`'s wasm32 branch hides the
                                    // "Save" button itself, so this variant is never actually
                                    // produced there in practice — this arm only exists to keep
                                    // the match exhaustive on that target.
                                    #[cfg(not(target_arch = "wasm32"))]
                                    crate::debugger::tastudio_panel::TastudioAction::SaveBranch => {
                                        actions.push(MenuAction::TastudioSaveBranch);
                                    }
                                    #[cfg(target_arch = "wasm32")]
                                    crate::debugger::tastudio_panel::TastudioAction::SaveBranch => {}
                                }

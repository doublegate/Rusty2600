//! MOS 6507 (Atari 2600 / VCS CPU) — the `Cpu` struct and instruction dispatch.
//!
//! Documented + undocumented NMOS 6502 opcodes, cycle-exact against the
//! SingleStepTests/`65x02` corpus (`tests/cpu-timing/`) and Bruce Clark's
//! decimal-mode test. See `docs/cpu.md` for the spec.
...
impl Cpu {
    ...
    // completes. Reads with side effects fire twice on RMW ABS,X — e.g. the
    // RIOT's `INTIM` ($284) clears its underflow flag on every read
    // (`docs/riot.md`), so an RMW opcode addressed there clears it via the
    // dummy read before the real read ever happens.
    ...
    fn inc_addr<B: CpuBus>(&mut self, bus: &mut B, addr: u16) {
        ...
        // same address before writing the modified value. Visible on the
        // 2600 at any TIA write-strobe register (`WSYNC`, `RESPx`, `HMOVE`,
        // `CXCLR`...) — they strobe on ANY write regardless of value, so an
        // `INC`/`DEC`/`ASL`/etc. targeting one double-strobes it via this
        // dummy write, not just the real one (`docs/tia.md`).

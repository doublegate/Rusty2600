//! Build script for `rusty2600-cheevos`.
//!
//! Compiles the vendored `RetroAchievements` `rcheevos` C library (MIT,
//! under `vendor/rcheevos/`) into a single static archive linked into this
//! crate.
//!
...
//! the 2600-relevant `src/rhash/` files (`hash.c`, `hash_rom.c`, `md5.c` —
//! the Atari 2600 uses `hash_rom.c`'s generic whole-buffer MD5 path, the
//! same as most non-headered ROM formats; no console-specific hashing case
//! exists for it in the vendored source).
//!
//! Excluded (and disabled via compile defines) because the 2600 needs none
//! of them and they pull external deps (zlib) or large encrypted/disc
//! machinery:

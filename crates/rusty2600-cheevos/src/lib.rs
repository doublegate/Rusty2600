//! `rusty2600-cheevos` — a native-only, safe Rust wrapper around the vendored
...
//!   address map (pure, unit-tested) — the 2600's ONLY RAM is the RIOT's
//!   128 bytes, so this is the entirety of what RA can condition against.
//!
//! ## Memory-source agnostic
//!
//! This crate does **not** depend on `rusty2600-core`. The memory source is
//! a caller-supplied closure `&mut dyn FnMut(u16) -> u8` taking a 2600
//! CPU-bus address (`|bus_addr| bus.peek(bus_addr)`, e.g.
//! `rusty2600_core::Bus::peek`); the RA-flat translation is handled
//! internally.
//!
//! ## Platform
//!
...
pub use memory::ra_addr_to_riot;
...
mod smoke {
    ...
    fn create_drive_destroy() {
        ...
        // Stub memory: every bus address reads 0.
    ...
    /// through the RIOT-RAM map: load no game, but call `do_frame` with a
    /// counting closure. (With no game loaded rcheevos may not read memory, so
    /// we only assert the closure machinery is sound — it must compile, run,
    /// and not double-free the thread-local on nested calls.)
    #[test]

...
//!   `rc_client` call. The trampoline maps the RA flat address to a 2600 bus
//!   address ([`crate::memory::ra_addr_to_riot`]) and calls the closure.
...
use crate::memory::ra_addr_to_riot;
...
impl ReadGuard {
    fn new<'a>(closure: &'a mut (dyn FnMut(u16) -> u8 + 'a)) -> Self {
        let ptr: *mut (dyn FnMut(u16) -> u8 + 'a) = closure;
        ...
        // never outlive `'a`.
        #[allow(clippy::transmute_ptr_to_ptr)] // erasing 'a, not a same-type no-op transmute
...
extern "C" fn read_trampoline(
    ...
    let result = std::panic::catch_unwind(|| {
        ...
        let closure: &mut dyn FnMut(u16) -> u8 = unsafe { &mut *ptr };
        let mut written = 0u32;
        for i in 0..num_bytes {
            match ra_addr_to_riot(address + i) {
                Some(bus_addr) => {
                    let byte = closure(bus_addr);
...
pub struct RaAchievement {
    /// The achievement's numeric id.
    pub id: u32,
    /// The achievement's display title.
    pub title: String,
    /// The achievement's unlock-condition description shown to the player.
    pub description: String,
    /// The point value awarded for earning this achievement.
    pub points: u32,
    ...
    pub bucket: u8,
    /// The measured-progress value as a percentage (0.0..=100.0), for
    /// progress-tracked achievements (e.g. "collect 50 coins").
    pub measured_percent: f32,
    /// The measured-progress value formatted as RA's own display string
    /// (e.g. `"12/50"`).
...
pub struct RaLeaderboard {
    /// The leaderboard's numeric id.
    pub id: u32,
    /// The leaderboard's display title.
    pub title: String,
    /// The leaderboard's description shown to the player.
    pub description: String,
    /// `RC_CLIENT_LEADERBOARD_STATE_*` raw value.
    pub state: u8,
    /// `RC_CLIENT_LEADERBOARD_FORMAT_*` raw value.
    pub format: u8,
    /// `true` if a lower tracked value ranks higher (e.g. a race's time),
    /// `false` if a higher value ranks higher (e.g. a score).
...
pub struct RaGameSummary {
    /// The number of achievements in the game's core (official) set.
    pub num_core_achievements: u32,
    /// The number of achievements in the game's unofficial set.
    pub num_unofficial_achievements: u32,
    /// The number of achievements the current user has unlocked.
    pub num_unlocked_achievements: u32,
    /// The number of achievements rcheevos flags as unsupported for this
    /// session (e.g. a hardcore-incompatible achievement while not in
    /// hardcore mode).
    pub num_unsupported_achievements: u32,
    /// The total point value of the core achievement set.
    pub points_core: u32,
    /// The point value the current user has unlocked so far.
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct RaUser {
    /// The user's display name (may differ from `username`, e.g. casing).
    pub display_name: String,
    /// The user's login username.
    pub username: String,
    /// The login token (persist this to log in without a password next time).
    pub token: String,
    /// The user's total hardcore RA score.
    pub score: u32,
    /// The user's total softcore RA score.
...
impl RaClient {
    ...
    ///
    /// `read` maps a 2600 CPU-bus address (`$0000..=$1FFF`, the 6507's
    /// 13-bit address space) to a byte — the caller's closure is typically
    /// `|bus_addr| bus.peek(bus_addr)`. The RA-flat -> bus-address mapping
    /// is handled internally.
    pub fn do_frame(&mut self, read: &mut dyn FnMut(u16) -> u8) {
    ...
    /// through `read`.
    pub fn idle(&mut self, read: &mut dyn FnMut(u16) -> u8) {
        let _r = ReadGuard::new(read);
        let _t = TransportGuard::new(&self.transport);
        // SAFETY: valid client; guards keep the closure + transport alive.
    ...
    /// Begin loading a game from raw ROM bytes (console = Atari 2600). rcheevos hashes
    /// the bytes internally to identify the game. `on_done` fires during a later
    /// [`poll_http_completions`][Self::poll_http_completions] call.

...
pub enum RaEvent {
    ...
    ChallengeIndicator {
        /// The associated achievement's numeric id.
        id: u32,
    ...
    LeaderboardTracker {
        ...
        /// `Some(true)` = show, `Some(false)` = hide, `None` = update.
        show: Option<bool>,
        /// The tracked value formatted as RA's own display string.
        display: String,
    },
    ...
    ServerError {
        ...
        msg: String,
        /// The API endpoint the error came from.
        api: String,
    },
    /// An event type this wrapper does not model in detail.
    Other {
        /// The raw `rc_client_event_t` event-type discriminant.
        event_type: u32,
    },
}
...
// Intentionally cohesive: this is a single flat match over rc_client's event
// enum, translating each borrowed C variant into an owned RaEvent; splitting
// it apart would scatter tightly-coupled union-pointer decoding with no real
// reuse benefit (matches the same rationale already used for app.rs's
// resumed()/render() in the frontend crate).
...
fn translate_event(ev: &ffi::rc_client_event_t) -> RaEvent {
    ...
    match ev.r#type {
        ...
        ffi::RC_CLIENT_EVENT_LEADERBOARD_SCOREBOARD => translate_scoreboard(scoreboard()),
...
/// Build a [`RaEvent::LeaderboardScoreboard`] from the borrowed
/// `rc_client_leaderboard_scoreboard_t`, copying its top-entries array into
/// owned [`RaScoreboardEntry`] values.
///
/// # Safety
/// Caller guarantees `sb`, if present, is valid for the duration of the call
/// (rcheevos' contract for the scoreboard event).
fn translate_scoreboard(sb: Option<&ffi::rc_client_leaderboard_scoreboard_t>) -> RaEvent {

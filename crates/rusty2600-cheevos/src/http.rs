...
/// here `Rusty2600/<crate version>`; an unrecognized client gets the "unknown
/// emulator" warning and cannot earn hardcore unlocks. Setting this is the
/// prerequisite for RA to allowlist Rusty2600 server-side (see docs / the
...
/// `Rusty2600/1.0.0 rcheevos/12.3.0`.
...
mod tests {
    ...
    /// The `RetroAchievements` User-Agent must identify the client as `Rusty2600`
    ...
    /// regression (e.g. a stray "Rusty2600 v2" or a dropped rcheevos version).
    #[test]
    fn ra_user_agent_identifies_rustynes_with_versions() {
        // Leading client token: `Rusty2600/<version>`, version present (not bare).
        ...
        assert_eq!(name, "Rusty2600", "RA identifies the client by this name");
        assert!(!version.is_empty(), "client version must be present");
        assert!(
            !RA_USER_AGENT.contains("Rusty2600 v2") && !RA_USER_AGENT.contains("Rusty2600_v2"),

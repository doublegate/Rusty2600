...
### T-21-001 — Sparse / mirrored bus decode
...
- **Description:** implement `Bus::cpu_read/cpu_write`: TIA write regs ($00–$2C
  ...
  on unmapped/write-only reads. (Closes `T-PS-051/052`.)
...
### T-21-002 — Beam counters + HBLANK/visible + WSYNC release
...
- **Description:** `Tia::tick_color_clock` advances `color_clock` 0..228 and
  ...
  RDY at HBLANK end. (Refines the scaffold; closes part of `T-PS-023`.)
- **Acceptance:** `wsync_sets_and_hblank_clears_rdy` green; a frame is exactly
  228 color clocks × the region line count.
- **Dependencies:** T-21-001. **Complexity:** M.

### T-21-003 — TIA write-register decode
...
### T-21-004 — Playfield + player dot emission
...
- **Description:** emit one `(luma, chroma)` per visible color clock from the
  ...
  (GRPx + NUSIZ copies + REFPx mirror + VDELPx). (Closes part of `T-PS-021`.)
...
- **Dependencies:** T-21-002, T-21-003. **Complexity:** L.

### T-21-005 — Scanline-buffer golden harness
...
- **Dependencies:** T-21-004. **Complexity:** M.

### T-21-006 — WSYNC mid-instruction freeze point
...
- **Dependencies:** T-21-002. **Complexity:** M.

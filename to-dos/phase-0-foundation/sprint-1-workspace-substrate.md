...
### T-01-001 — Crate split compiles clean
...
### T-01-002 — `no_std` cross-compile gate for the chip stack
...
- **Dependencies:** T-01-001. **Complexity:** S.

### T-01-003 — Lockstep scheduler + seeded determinism phase
...
- **Dependencies:** T-01-001. **Complexity:** M (loop scaffolded; the exact
  mid-instruction freeze point is `T-PS-061`, deferred to Phase 2/6).

### T-01-004 — `Bus` decode skeleton (no WRAM field)

- **Description:** `Bus` owns tia/riot/board/open_bus and exposes `cpu_read/
  cpu_write` masking to 13 bits. The real sparse/mirrored decode is `T-PS-051/052`
...
### T-01-005 — Core cart boards + `detect()` + honesty gate
...
- **Dependencies:** T-01-001. **Complexity:** S (scaffolded).

### T-01-006 — Harness shapes present
...
### T-01-007 — STATUS + CI report v0.1.0
...
- **Dependencies:** T-01-001..006. **Complexity:** S.

...
full plan and `CHANGELOG.md`'s `[1.6.0]` entry). Adds a new
`rusty2600-thumb` crate: a real ARM7TDMI Thumb-1 interpreter ported from
Gopher2600's Go implementation (`hardware/memory/cartridge/arm/`), not
Stella's C++ `Thumbulator` — registers, N/Z/C/V status flags, the
...
hand-authored conformance tests. This release lands the interpreter core
only — it is **not yet wired into any `rusty2600-cart` `Board`/`Cartridge`
...
Earlier in the line: `v1.5.0 "Full Catalog"` implemented `Bank4A50`
...
| **Workspace test suite** | `cargo test --workspace` | **217 / 217** (both Klaus tests moved to `--features test-roms`, gated out of the fast default path — see `crates/rusty2600-test-harness/tests/klaus_test.rs`; +27 vs. v1.5.0 for the new `rusty2600-thumb` conformance tests) |
| **Workspace test suite (`--features test-roms`)** | `cargo test --workspace --features test-roms` | **220 / 220** |

...
into automatic `detect()`. **v0.4.0 "Breadth" is in progress**: Batches 1-2
of the BestEffort long tail (F0, E0, 3F, 3E, EF/EFSC, DF/DFSC, BF/BFSC) are
implemented, including a new `Board::snoop_write` hook so bankswitch schemes
that trigger on TIA/RIOT-mirrored addresses (not just the `$1000+` cart
window) can be modeled at all — see
`to-dos/phase-4-carts-mappers/sprint-2-besteffort-boards.md`. Phase 5
Frontend (rendering, pacing, input, WASM/thread support) has a working
synchronous AND dedicated emu-thread path (`emu-thread` default-on); its
debugger panels are still TODO stubs (targeted for v0.5.0).
...
| `rusty2600-cart` | Bankswitch boards | All 8 Curated schemes (2K, 4K, F8, F6, F4, CV, FA/CBS-RAM, Superchip, DPC, E7) implemented and wired into `detect()` (v0.3.0). BestEffort Batches 1-2 (v0.4.0, in progress): F0, E0, 3F, 3E, EF/EFSC, DF/DFSC, BF/BFSC implemented and wired (17 of 25 catalogued schemes total). A new `Board::snoop_write` hook (`crates/rusty2600-core/src/bus.rs`) lets boards react to writes the console routes to TIA/RIOT space — needed for 3F/3E's `$3E`/`$3F` hotspots, which aren't in the cart window at all. SB/X07/4A50/UA/0840/FE need the same treatment on READS too (`T-0402-006`/`011`, deferred — a bigger interface question, not rushed). The remaining ~32-scheme BestEffort long tail (Batches 3-5) targets the rest of v0.4.x. |
...
| **Workspace test suite** | `cargo test --workspace` | **119 / 119** (both Klaus tests moved to `--features test-roms`, gated out of the fast default path — see `crates/rusty2600-test-harness/tests/klaus_test.rs`) |
| **Workspace test suite (`--features test-roms`)** | `cargo test --workspace --features test-roms` | **121 / 121** |
...
| BestEffort | 15 | F0, FE, E0, 3F, 3E, UA, 0840, EF, BF, DF, SB, X07, 4A50, AR, DPC+/CDF/CDFJ | **no** | F0, E0, 3F, 3E (Batch 1, `T-0402-001..004`) + EF/EFSC, DF/DFSC, BF/BFSC (Batch 2, `T-0402-008..010`), all wired into `detect()`. UA/0840/FE/SB/X07/4A50 need `snoop_read` (`T-0402-006`/`011`); the remaining Batches 3-5 (~32 schemes) target the rest of v0.4.x. |
